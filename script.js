...
const sunset = document.querySelector(".record2")
const visibility = document.querySelector(".visibility")
const cel = document.querySelector(".cel")
const fah = document.querySelector(".fah")
const perc = document.querySelector(".perc")
const searchBtn=document.querySelector(".locbtn")
...
const city = "Bengaluru"; // Replace with the city of your choice
...
fetch(weatherApiUrl)
 ...
 const todayTem23=document.querySelector(".heat23")
  ...
  // code to display location 
    ...
    // code to display the change icons
    const btn3=res.currentConditions
    changeIcon(btn3.icon)
    ...
    // code to access the Air Quality
    const airQualityValue=res.currentConditions.airquality1
    displayAirQuality(airQualityValue)
    ...
    sunrise.textContent = `${res.currentConditions.sunrise} am`;
    sunset.textContent=`${res.currentConditions.sunset} pm`
...
// code to display the weekly icons change
...
function displayHumidity(humidity){
  ...
  if(humidity>=0 && humidity<30){
    ...
    humiValue='Very High'
  }else if(humidity>=91 && humidity<=100){
    humiValue="Hazardous"
  }
...
function displayVisibility(visibility){
  ...
  let visibilityValue=''; 
  if(visibility>=0 && visibility<=0.03){
    visibilityValue='Dense Fog'
  }else if(visibility>=0.04 && visibility<=0.16){
    visibilityValue='Moderate Fog'
  }else if(visibility>=0.17 && visibility<=0.35){
    visibilityValue='Light Fog'
  }else if(visibility>=0.36 && visibility<=1.13){
    visibilityValue='Very Light Fog'
  }else if(visibility>=1.14 && visibility<=2.16){
    visibilityValue="Light Mist"
  }else if(visibility>=2.17 && visibility<=5.4){
    visibilityValue="Very Light Mist"
  }else if(visibility>5.41 && visibility<=10.8){
    visibilityValue="Clear Air"
  }else if(visibility>=10.81 && visibility> 10.81){
    visibilityValue="Very Clear Air"
  }
  visibilityElement.textContent=`${visibilityValue}`
}

//code to display Air quality

function displayAirQuality(airquality1){
const airquality=document.querySelector(".airqua")
  let airqualityValue=''
  if(airquality1>=0 && airquality1<=0.03){
    airqualityValue="Good👌"
  }else if(airquality1<=0.04 && airquality1<=0.16){
    airqualityValue="Moderate 😐"
  }else if(airquality1<=0.17 && airquality1<=0.35){
    airqualityValue="Unhealthy for Sensitive Groups 😷"
  }else if(airquality1<=0.36 && airquality1<=1.13){
    airqualityValue="Unhealthy 😷"
  }else if(airquality1<=1.14 && airquality1<=2.16){
    airqualityValue="Very Unhealthy 😨"
  }else if(airquality1<=2.17 && airquality1<=5.4){
    airqualityValue="Hazardous 😱"
  }else if(airquality1<=5.41 && airquality1<=10.8){
  }else if(airquality1<=10.81 && airquality1> 10.81){
  }
